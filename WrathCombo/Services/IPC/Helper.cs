...
public partial class Helper(ref Leasing leasing)
{
    ...
    internal static Preset? GetOppositeModeCombo(Preset preset)
    {
        ...
        if (pdata.ComboType is not (ComboType.Advanced or ComboType.Simple))
            return null;

        // Detect the target type
        var targetType =
            pdata.Name.Contains("single target", lower)
                ? ComboTargetTypeKeys.SingleTarget
                : (pdata.Name.Contains("- aoe", lower))
        ...
        var simplicityLevel =
            pdata.ComboType is ComboType.Simple
